...
class AprioriController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $data['ts3'] = $request->ts3;
        $data['tc3'] = $request->tc3;
    ...
    public function combineThirdProccess(Request $request)
    {
        $data = [];
        $totalData = 0;
        $totalDataFiltered = 0;
            ...
            foreach ($findData as $dt) {
                ...
                foreach ($findData2 as $dt2) {
                    ...
                    foreach ($findData3 as $dt3) {
                        $prd3 = $dt3->product_id;
                        $findData3 = TRSLIST::whereIn('product_id', [$prd1, $prd2, $prd3])->groupBy('product_id');
                        $counts = $findData3->count();

                        $findDataConfidence = TRSLIST::whereIn('product_id', [$prd1, $prd2])->groupBy('product_id');
                        $countsConfidence = $findDataConfidence->count();
                        ...
                        $data[] = [
                            'name'    => $dt->name . " &nbsp;,&nbsp; " . $dt2->name . " &nbsp;,&nbsp; " . $dt3->name,
                            'support' => "Support = " . $counts . " / " . $sumAlltransF . " = " . $gsupports,
                            'confidence' => "Confidence = " . $counts . " / " . $countsConfidence . " = " . $gconfides,
                        ];
                    }
    ...
    public function combineThirdRules(Request $request)
    {
        $data = [];
        $totalData = 0;
        $totalDataFiltered = 0;
        ...
        $setSup3 = $request->setSup3;
        $setSupxConf3 = $request->setSupxConf3;
            ...
            foreach ($findData as $dt) {
                ...
                foreach ($findData2 as $dt2) {
                    ...
                    foreach ($findData3 as $dt3) {
                        ...
                        $data[] = [
                            'rules'      => "If buy &nbsp; <i>" . $dt->name . " </i> &nbsp; And buy &nbsp; <i>" . $dt2->name . "</i>" . "  &nbsp; Then buy &nbsp; <i>" . $dt3->name . "</i>",
                            'support'    => $gsupports,
                            'confidence' => $gconfides,
                            'supxconf'   => $supxconf,
                            'tsup'       => $tsup,
        ...
        foreach ($findData as $dt) {
            ...
            foreach ($findData2 as $dt2) {
                ...
                foreach ($findData3 as $dt3) {
                    ...
                    $findDataFinals = TRSLIST::whereIn('product_id', [$prd1, $prd2, $prd3])
                        ->groupBy('product_id');
                    $counts = $findDataFinals->count();

                    $findDataConfidence = TRSLIST::whereIn('product_id', [$prd1, $prd2])->groupBy('product_id');
                    $countsConfidence = $findDataConfidence->count();

                    $gsupport = (float)$counts / $sumAlltransF;
                    $gconfide = (float)$counts / $countsConfidence;

                    $data[] = [
                        'ProductID' => $prd1 . "," . $prd2  . "," . $prd3,
                        'ProductName' => $dt->name . "," . $dt2->name . "," . $dt3->name,
                        'Count' =>  $counts,
                        'Support' => number_format((float)$gsupport, 2, '.', ''),
                        'ConfidenceGet' => $countsConfidence,
                        'Confidence' => number_format((float)$gconfide, 2, '.', ''),
